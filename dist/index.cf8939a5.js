...
function submitScore(mode, name, score) {
    ...
    }).then((res)=>res.json()).then((data)=>console.log("Score submitted", data)).catch((err)=>console.error("Error submitting score", err));
}
function fetchLeaderboard(mode, callback) {
    fetch(`https://dodgetheblock.netlify.app/.netlify/functions/get-leaderboard?mode=${mode}`).then((res)=>res.json()).then((data)=>callback(data.leaderboard)).catch((err)=>{
...
function formatLeaderboardFromData(data) {
    ...
    for(let i = 0; i < data.length; i++)text += `${i + 1}. ${data[i].name} - ${data[i].score}\n`;
    return text;
}
...
// -------------------------
function create() {
    background = this.add.image(0, 0, 'background').setOrigin(0).setDisplaySize(config.width, config.height);
...
function update(time, delta) {
    ...
    let dt = delta / 16.67;
    // Player Movement
    if (cursors.left.isDown) player.x -= playerSpeed * dt;
    else if (cursors.right.isDown) player.x += playerSpeed * dt;
    if (targetX !== null) {
        let diff = targetX - player.x;
        if (Math.abs(diff) > playerSpeed) player.x += Math.sign(diff) * playerSpeed * dt;
    ...
    blocks.getChildren().forEach(function(block) {
        ...
        if (checkCollision(player, block)) showGameOver();
    ...
    if (selectedMode === "normal") {
        if (score >= nextMaxBlocksIncreaseScore) {
            maxBlocks = Phaser.Math.Between(modeSettings.normal.initialMinBlocks, modeSettings.normal.initialMaxBlocks + Math.floor(score / 20) * modeSettings.normal.maxBlockIncrease);
            nextMaxBlocksIncreaseScore += threshold * 2;
        }
    } else if (selectedMode === "asian") {
        if (score >= nextMaxBlocksIncreaseScore) {
            maxBlocks = Phaser.Math.Between(modeSettings.asian.initialMinBlocks, modeSettings.asian.initialMaxBlocks + Math.floor(score / threshold) * modeSettings.asian.maxBlockIncrease);
...
function createModeSelectionUI(scene) {
    ...
    let personalHighscoreText = scene.add.text(0, 50, `Your Highscore:\nNormal: ${personalHighscoreNormal}   Asian: ${personalHighscoreAsian}`, {
        fontSize: '14px',
        fill: '#fff',
        align: 'center'
    }).setOrigin(0.5);
    let leaderboardNormalText = scene.add.text(-config.width / 4, 80, "Normal:\n" + formatLeaderboard("normal"), {
        fontSize: '14px',
        fill: '#fff',
        align: 'center'
    }).setOrigin(0.5, 0);
    let leaderboardAsianText = scene.add.text(config.width / 4, 80, "Asian:\n" + formatLeaderboard("asian"), {
        fontSize: '14px',
        fill: '#fff',
        align: 'center'
    }).setOrigin(0.5, 0);
...
function startGame(scene) {
    ...
    player = scene.add.image(config.width / 2, config.height - 80, 'player').setOrigin(0.5).setDisplaySize(40, 40);
    score = 0;
    ...
    if (selectedMode === "normal") maxBlocks = Phaser.Math.Between(modeSettings.normal.initialMinBlocks, modeSettings.normal.initialMaxBlocks);
    else if (selectedMode === "asian") maxBlocks = Phaser.Math.Between(modeSettings.asian.initialMinBlocks, modeSettings.asian.initialMaxBlocks);
...
function spawnBlock() {
    ...
    if (blocks.getLength() < maxBlocks) {
        let newBlock = this.add.image(Phaser.Math.Between(40, config.width - 40), 0, 'block').setOrigin(0.5).setDisplaySize(40, 40);
        if (selectedMode === "normal") newBlock.baseSpeed = Phaser.Math.Between(modeSettings.normal.blockSpeedMin, modeSettings.normal.blockSpeedMax);
        else if (selectedMode === "asian") newBlock.baseSpeed = Phaser.Math.Between(modeSettings.asian.blockSpeedMin, modeSettings.asian.blockSpeedMax);
...
function createGameOverUI(scene) {
    ...
    let gameOverBg = scene.add.image(0, 0, 'gameOverBg').setOrigin(0.5).setDisplaySize(300, 200);
    ...
    let restartButton = scene.add.image(0, 50, 'restartButton').setOrigin(0.5).setInteractive();
...
function showGameOver() {
    ...
    if (selectedMode === "normal") {
        ...
        if (score > hs) localStorage.setItem('highscore_normal', score);
        updateLeaderboard("normal", playerName, score);
    } else if (selectedMode === "asian") {
        let hs = localStorage.getItem('highscore_asian') || 0;
        if (score > hs) localStorage.setItem('highscore_asian', score);
...
//# sourceMappingURL=index.cf8939a5.js.map
